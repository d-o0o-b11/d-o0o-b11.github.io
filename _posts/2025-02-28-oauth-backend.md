...
현재는 DB에 Refresh Token을 저장하고 있지만 유효기간 설정이 없어 사용자가 명시적으로 로그아웃하거나 다시 로그인해야만 토큰이 갱신되는 구조입니다.  
후에 Redis와 같은 In-Memory DB를 활용해서 `유저 ID - 기기 ID`를 키로 설정하고 Refresh Token의 유효기간과 동일하게 저장하는 방식으로 개선하는 것도 고려 중입니다.

이렇게 하면 토큰 유출이 발생하더라도 로그아웃만으로 쉽게 무효화할 수 있고 유효기간이 지나면 Redis에서 자동으로 삭제되므로 관리도 더 효율적입니다.

[Redis 기반 인메모리 세션 + Refresh Token 구조 설계 글 보러가기](http://d-o0o-b11.github.io/posts/in-memory-rt/)
